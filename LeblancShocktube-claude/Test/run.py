...
for P in plist:
    ...
    tree = ET.parse('Leblanc1DSession.xml')
    root = tree.getroot()
    ...
    add_geometry(root, elements=Nel, x0=0.0, x1=10.0)

    ET.indent(tree, space="\t", level=0)

    # Saving .xml
    tree.write(f"P{P}/Leblanc_{P}.xml", encoding="utf-8", xml_declaration=True)

    # Running simulation
    cmprsblflw = f"CompressibleFlowSolver Leblanc_{P}.xml"
    if run:
    ...
    for t in range(TFinal):
        generate_csv(f"P{P}", f"Leblanc_{P}.xml", f"Leblanc_{P}_{t}.chk", t)
...
for t in range(TFinal):
    ...
    rho, u_vel, p, E = leblanc_exact(x, 3, t*dt)
    ...
    fig.suptitle(fr'$N_{{el}} = {Nel}, t = {t*dt:.1f}, \mu_0 = {mu0}$')
