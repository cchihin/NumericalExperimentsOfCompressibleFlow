...
for upwd in upwdlist:

    # Creating folders
    if os.path.exists(f"{upwd}"):
        print(f"{upwd}/ exists")
    else:
        os.mkdir(f"{upwd}")
    ...
    # Modifying P
    # Change the UpwindType
    for item in root.findall(".//SOLVERINFO/I"):
        if item.get("PROPERTY") == "UpwindType":
            item.set("VALUE", upwd)    # or whatever scheme you want
            print(f"Updated UpwindType to {upwd}")
    ...
    # Saving .xml
    tree.write(f"{upwd}/SodTube_{upwd}.xml", encoding="utf-8", xml_declaration=True)

    # Running simulation
    cmprsblflw = f"CompressibleFlowSolver SodTube_{upwd}.xml"
    if run:
        subprocess.run(cmprsblflw.split(), cwd=f"{upwd}")
    else:
        print(cmprsblflw)

    # # Processing 
    for t in range(5):
        generate_csv(f"{upwd}", f"SodTube_{upwd}.xml", f"SodTube_{upwd}_{t}.chk", t)
...
for t in range(5):
    ...
    for upwd in upwdlist:
        data = np.genfromtxt(f'{upwd}/points_{t}.csv', delimiter=',', skip_header=1)

        ax[0,0].plot(x, rho, 'k--')
        ax[0,0].plot(data[:,0], data[:,2], label=f"{upwd}")
